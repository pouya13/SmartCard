...
uint16_t SmartCardReader::sendAPDU(APDU_t* command, boolean send, uint8_t maxwaits) {
  ...
  uint8_t waitsleft = maxwaits; //CJT
      ...
      switch(state) {
        case SEND_HEADER:
          ...
          if (command->data_size == 0 || command->data_buf == NULL) {
            bytes_count = 5; // CJT - was 4: you must send 5 bytes header
            buf[4] = 0;
        ...
        case WAIT_ANSWER:      
          ...
          if ((bytes_received = receiveBytes(buf, 1)) > 0) {
            ...
            // CJT If the card is doing something big like creating or deleting a big file
            // it can take while to respond, don't just assume its gone mute		  
            maxwaits--;
            break;
		  }
          result = 0;
